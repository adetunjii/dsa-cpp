...
int main() {
    ConsistentHashing ch(6); // Number of virtual nodes per real node
    ...
    cout << "Key 'samuel' is assigned to node: " << ch.getNode("samuel") << endl;

    ch.removeNode("Node2");

    cout << "After removing 'Node2', key 'mykey' is assigned to node: " << ch.getNode("mykey") << endl;
    cout << "After removing 'Node2', key 'samuel' is assigned to node: " << ch.getNode("samuel") << endl;
